...
const Receivables = () => {
  const { receivables, getAllReceivables } = useContext(EntriesContext);
  var totalAmount=0
  useEffect(() => {
    getAllReceivables();
  }, []);
  ...
  return (
    ...
    <div className="flex justify-center items-center  ">
      <div className="absolute top-0 text-center ml-52 mt-2 font-semibold text-lg dark:text-gray-300"> Receivables</div>
        ...
        <tbody>
          {receivables.map((entry) => {
            ...
            <tr key={entry._id} className={`${entry.journalEntry==="rr"?"bg-red-200 border-b dark:text-gray-600 text-gray-600 border-gray-300":entry.journalEntry==="dr"?"bg-green-200 border-b dark:text-gray-600 text-gray-600 border-gray-300":entry.journalEntry==="cr"?"bg-yellow-200 border-b dark:text-gray-600 text-gray-600 border-gray-300":""}`}>
              ...
              <td className="px-20 py-4">{entry.journalEntry==="cr"?entry.Description+" (Customer A/c)":entry.Description}</td>
           ...
           <tr  className="bg-gray-200 border-b dark:text-gray-600 text-gray-600 border-gray-300">
              ...
              <td className="px-20 py-4 font-semibold">Receivable:</td>
