...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const { name, description, category, price, quantity, vendorPrice, vendor, vendorId, paid } =
        req.body
...
router.get("/fetchAll", validator, async (req, res) => {
  try {
    if (
      req.user.role === "admin" ||
      req.user.role === "inventory manager" ||
...
router.delete("/delete/:id", validator, async (req, res) => {
  ...
  try {
    if (req.user.role === "admin" || req.user.role === "inventory manager") {
      ...
      if (productFromDb.user.toString() !== req.user.id)
